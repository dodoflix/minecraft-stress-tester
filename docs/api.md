...
| POST | `/api/script/validate` | Parse-check user JS in an isolate without running it. Body: `{ code }`. Returns `{ ok, error? }`. |
| POST | `/api/script/run` | Connect one bot and run user JS against it in an isolate, returning its logs. Body: `{ code, target: { host, port }, authorized, timeoutMs? }`. |
| POST | `/api/graph/compile` | Compile a visual node graph to a blueprint plus ejected code. Body: `{ graph }`. |
| POST | `/api/graph/import` | Turn a blueprint into a node graph. Body: `{ blueprint }`. |
